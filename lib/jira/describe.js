        ...
        .end(function (res) {
          if (!res.ok) {
            return console.log(res.body.errorMessages.join('\n'));
              ...
              request
                .get(config.auth.url + "rest/api/2/issue/" + issue + "/remotelink")
                .set('Content-Type', 'application/json')
                .set('Authorization', 'Basic ' + config.auth.token)
                .end(function (resLink) {

                  console.log('Related Links to this issue:');

                  var table = new Table({
                    head: ['Key', 'Name']
                  });
                  resLink.body.forEach(function(data, index) {
                    table.push([data.object.title, data.object.url]);
                  });
                  console.log(table.toString());

                  console.log('Other detail info to this issue:');
